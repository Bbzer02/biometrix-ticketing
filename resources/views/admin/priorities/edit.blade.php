...
@section('content')
    <div class="rounded-2xl border border-blue-200 bg-blue-50/40 p-6 shadow-sm sm:p-8 dark:border-blue-400/40 dark:bg-blue-500/10">
        <h1 class="mb-6 text-xl font-semibold text-slate-900 sm:text-2xl">Edit priority</h1>

        <form action="{{ route('admin.priorities.update', $priority) }}" method="post" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="key" id="key" value="{{ old('key', $priority->key) }}" required class="input-ticket">
                    @error('key')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="label" class="label-ticket">Label</label>
                    <input type="text" name="label" id="label" value="{{ old('label', $priority->label) }}" required class="input-ticket">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" class="input-ticket min-h-[100px] resize-y">{{ old('description', $priority->description) }}</textarea>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $priority->sort_order) }}" min="0" class="input-ticket">
                    @error('sort_order')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="pt-6">
                    <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                        <input type="checkbox" name="active" value="1" {{ old('active', $priority->active) ? 'checked' : '' }}>
                        Active
                    </label>
